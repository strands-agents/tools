...
from strands_tools import image_reader, nova_canvas
...
def test_remove_background(agent, tmp_path):
    ...
    # 3. test semantic usage to check if it recognizes dog/corgi
    semantic_result = agent(f"Has the background been removed from the image at `{image_path_no_bg} \
                             - compare with image at {image_path}` respond with yes or no first")
...
def test_virtual_try_on_prompt_mask(agent, tmp_path):
    # 1. Generate an image of an empty living room
    prompt = "an empty room with a white background and a purple couch in the middle"
    image_gen_result = agent.tool.nova_canvas(
        text=prompt,
        task_type="TEXT_IMAGE",
        model_id="amazon.nova-canvas-v1:0",
    ...
    # Save the couch image to temp directory
    couch_image_path = tmp_path / "green_couch.png"
    with open(couch_image_path, "wb") as f:
        f.write(found_image)
