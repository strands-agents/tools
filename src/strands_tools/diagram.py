...
def render_as_mermaid(nodes, edges, title, style):
    """Convert nodes/edges to Mermaid format"""
    direction = "LR" if style.get("rankdir") == "LR" else "TD"
...
def render_as_ascii(nodes, edges, title, style):
    """Convert nodes/edges to ASCII format"""
    lines = [f"# {title}", ""]
    ...
    lines.append("")
    for edge in edges:
        arrow = " --> " if style.get("rankdir") == "LR" else " ↓ "
...
class DiagramBuilder:
    ...
    def render(self, diagram_type: str, output_format: str) -> str:
        ...
        if output_format in ["mermaid", "ascii"]:
            return self._render_text(output_format)
    ...
    def _render_text(self, output_format: str) -> str:
        ...
        if output_format == "mermaid":
            content = render_as_mermaid(self.nodes, self.edges, self.title, self.style)
            ext = ".md"
        else:  # ascii
            content = render_as_ascii(self.nodes, self.edges, self.title, self.style)
            ext = ".txt"

        output_path = os.path.join(os.getcwd(), f"{self.title}{ext}")
    ...
    def _render_cloud(self, output_format: str) -> str:
        ...
        nodes_dict = {}
        output_path = os.path.join(os.getcwd(), self.title)
    ...
    def _render_network(self, output_format: str) -> str:
        ...
        output_path = os.path.join(os.getcwd(), f"{self.title}.{output_format}")
    ...
    def _render_sequence(self, output_format: str) -> str:
        ...
        # Sort edges by order
        sorted_edges = sorted(self.edges, key=lambda x: x.get("order", 0))

        # Position participants horizontally
        participant_positions = {node["id"]: i for i, node in enumerate(self.nodes)}
        participant_labels = {node["id"]: node.get("label", node["id"]) for node in self.nodes}
        ...
        for i, node in enumerate(self.nodes):
            ...
            ax.text(
                i,
                len(sorted_edges) + 0.5,
                participant_labels[node["id"]],
                ha="center",
                va="bottom",
                fontweight="bold",
                fontsize=10,
        ...
        ax.set_xlim(-0.5, len(self.nodes) - 0.5)
...
def create_cloud_diagram(nodes, edges, title, output_format, style):
    """Legacy function - use DiagramBuilder instead"""
    builder = DiagramBuilder(nodes, edges, title, style)
    return builder._render_cloud(output_format)
...
def create_graph_diagram(nodes, edges, title, output_format, style):
    ...
    builder = DiagramBuilder(nodes, edges, title, style)
    if output_format in ["mermaid", "ascii"]:
        return builder._render_text(output_format)
    return builder._render_graph(output_format)
...
def create_network_diagram(nodes, edges, title, output_format, style):
    ...
    if output_format in ["mermaid", "ascii"]:
        return builder._render_text(output_format)
    return builder._render_network(output_format)


def create_sequence_diagram(nodes, edges, title, output_format, style):
    """Legacy function - use DiagramBuilder instead"""
    builder = DiagramBuilder(nodes, edges, title, style)
    return builder._render_sequence(output_format)
...
def diagram(
    ...
    Args:
        ...
        output_format: Output format ("png", "svg", "pdf", "mermaid", "ascii")
                      Note: "mermaid" and "ascii" formats are only supported for "graph" and "network" diagram types
