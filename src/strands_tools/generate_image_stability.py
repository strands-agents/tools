...
TOOL_SPEC = {
    ...
    "description": "Generates an image using Stability AI's latest models on the Stability Platform, including SD3.5, Stable Image Ultra and Stable Image Core.",
    "inputSchema": {
        "type": "object",
        "properties": {
            "stability_api_key": {
                "type": "string",
                "description": "Your Stability API key for authentication. Required for accessing the Stability AI API.",
            },
            "return_type": {
                "type": "string",
                "description": "The format of the returned image. Can be 'json' for JSON response with base64 data or 'image' for raw image bytes. Default is 'image'.",
                "enum": ["json", "image"],
                "default": "image"
            },
            "prompt": {
            ...
            "model_id": {
                "type": "string",
                "description": "The model to use for image generation. One of: 'stability.stable-image-core-v1:1', 'stability.stable-image-ultra-v1:1', 'stability.sd3-5-large-v1:0'",
                "enum": [
            ...
            "aspect_ratio": {
                "type": "string",
                "description": "Controls the aspect ratio of the generated image. This parameter is only valid for text-to-image requests. For image-to-image requests, the aspect ratio is determined by the input image.",
                "enum": ["16:9", "1:1", "21:9", "2:3", "3:2", "4:5", "5:4", "9:16", "9:21"],
                "default": "1:1"
            },
            "seed": {
                "type": "integer",
                "description": "Optional: Seed for random number generation. Omit this parameter or pass 0 to use a random seed.",
                "minimum": 0,
                "maximum": 4294967294,
                "default": 0
            },
            "cfg_scale": {
                "type": "number",
                "description": "CFG scale for image generation (only used for stability.sd3-5-large-v1:0). Controls how closely the image follows the prompt. Higher values mean stricter adherence. Range: 1.0-10.0",
                "minimum": 1.0,
                "maximum": 10.0,
                "default": 4.0
            },
            ...
            "image": {
                "type": "string",
                "description": "Input image for img2img generation. Should be base64-encoded image data in jpeg, png or webp format."
            },
            "mode": {
                "type": "string",
            ...
            "strength": {
                "type": "number",
                "description": "Sometimes referred to as denoising, this parameter controls how much influence the image parameter has on the generated image. A value of 0 would yield an image that is identical to the input. A value of 1 would be as if you passed in no image at all. Only used when mode is image-to-image.",
                "minimum": 0.0,
                "maximum": 1.0
            },
            "negative_prompt": {
                "type": "string",
                "description": "Text describing what you do not want to see in the generated image. Helps exclude unwanted elements or styles.",
                "maxLength": 10000
            }
        },
...
def call_stability_api(
    ...
    Args:
        ...
        return_json: Whether to return JSON response (deprecated, use return_type)
    ...
    if image:
        ...
        try:
            ...
            # If not base64, assume it's a URL or file path
            files["image"] = image
...
def call_stability_api(
    ...
    This is an internal helper function that handles the actual API call.
    ...
    if cfg_scale is not None:
        data["cfg_scale"] = cfg_scale
    if seed is not None and seed > 0:  # Only add seed if it's not 0 (random)
        data["seed"] = seed
    if style_preset:
        data["style_preset"] = style_preset
    if strength is not None and mode == "image-to-image":
