...
def test_record_memory(provider, mock_elasticsearch_client, mock_bedrock_client):
    ...
    mock_elasticsearch_client["client"].index.return_value = {
        "result": "created",
        "_id": "test_memory_id"
    }
...
def test_get_memory(provider, mock_elasticsearch_client):
    ...
    mock_elasticsearch_client["client"].get.assert_called_once_with(
        index="test_index",
        id="mem_123"
    )
...
def test_delete_memory(provider, mock_elasticsearch_client):
    ...
    mock_elasticsearch_client["client"].delete.return_value = {
        "result": "deleted"
    }
    ...
    mock_elasticsearch_client["client"].delete.assert_called_once_with(
        index="test_index",
        id="mem_123"
    )
...
def test_pagination_support(provider, mock_elasticsearch_client):
    ...
    result = provider.elasticsearch_memory(action="list", max_results=5, next_token="10")
